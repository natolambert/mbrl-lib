...
class PolicyWrapper:
    ...
    when using and training reactive policies.
    ...
    Args:
        ...
        obs_process_fn (callable, optional): if provided, observations will be passed through
            ...
            Defaults to ``None``.
        gaussian (bool): if provided, the policy will take the form of a gaussian, with a
            variance over each chosen action.
    """

    _MODEL_FNAME = "policy.pth"
    ...
    def _get_model_input_from_np(
        self, obs: np.ndarray, device: torch.device
    ) -> torch.Tensor:
        if self.obs_process_fn:
            obs = self.obs_process_fn(obs)
        model_in_np = obs  # np.concatenate([obs], axis=1)
        if self.normalizer:
    ...
    def _get_model_input_from_tensors(self, obs: torch.Tensor):
        if self.obs_process_fn:
            obs = self.obs_process_fn(obs)
        model_in = obs  # torch.cat([obs], axis=1)
        if self.normalizer:
    ...
    def _get_model_input_and_target_from_batch(
        ...
        model_in = self._get_model_input_from_np(obs, self.device)
        target = torch.from_numpy(action).to(self.device)
    ...
    def predict(
        ...
        """
        model_in = self._get_model_input_from_tensors(obs)
        ...
        #     next_observs = tmp_
        rewards = None  # predictions[:, -1:] if self.learned_rewards else None
...
class PolicyTrainer:
    ...
    Args:
        policy (:class:`mbrl.models.PolicyWrapper`): the wrapper to access the
            model to train.
    ...
    _LOG_GROUP_NAME = "policy_train"

    def __init__(
        self,
        dynamics_model: PolicyWrapper,
